...
tags:
  ...
  - C/C++
date: 2022-08-10
excerpt: The high-level archetecture of a modern compiler.
---

<small>*The following is summarized from: Crafting Interpreters by 
<a href="https://stuffwithstuff.com" target="_blank">Robert Nystrom</a>*</small>

<Image src="/posts/The-Anatomy-of-a-Compiler/language-structure.svg" 
    width={3440}
    height={1440}
/>
...
lexical analysis). The **scanner** injests the source code as a stream of 
raw characters and chunks them together to for tokens. A **token** is a 
group of characters (of size n >= 1) that have a collective meaning.

Some tokens are a single character (i.e. \{ and ,) while others can be several
character long, such as numbers (123), string literals ("hi"), and
identifiers (min). 

Some characters from the source code don't have any significace to the
scanner such as whitespace and comments. These things are only usefull to 
...
Scanning is a ubiqitous task done for every language. Open source tools have 
been created that take a predefined grammer and output a fully 
functioning scanner. See [flex](https://github.com/westes/flex)

## Parsing
The parser provides the language with it's sytax by utilizeing rules outlined 
in the languages **grammar**. Much like in linguistic languages, the grammer
...
<Image src="/posts/The-Anatomy-of-a-Compiler/AST.svg"
    width={3440}
    height={1440}
/>
...
of attributes or in a seperate lookup table using the identifiers as keys. 
If an even more powerful form of bookkeyping is required, one may transform 
the tree into an entirly new data structure called an Intermedite 
Representation (IR). This will be coverd in the next section.

Everything up to this point is referred to as the language's **front end**.
This is where all of the language specics occur. From this terminology you 
probably deduced that the language also has a back end. The language's 
**back end** is concerned with the final archetecure where the code will run.
...
to think to hard about that name).
...
The **Intermediate represention (IR)** is part of the middle end. Esentially, 
it's a standardized representaion of the data that enables you to mix and match
different front ends and back ends. This way you could reduce your work load 
significantly by reusing parts of the pipeline, or you can elimnate work 
entirly by utilizing pre-written backends for a large swoth or archetectures.
...
in order to make the code more efficient once it's compiiled down to it's 
...
There are ultimatly two paths one can take, do you wan't to generate code 
...
hypothecial (idealized) machine. It's called bytecode becuase each instruction 
...
earch archetecure we'd like to support that converts the bytecode to native
code, this will have to be done for every chip that needs to be supported but 
we'll get to reuse the rest of the compiler pipeline accross all the machines 
...
slower than running native code, however it offeres the upsides of being 
much more simple and portable.

## Runtime
It's time to run the compiled program. If you compiled down to native machine 
code, you can just tell your OS to run the program, if you went the VM route 
